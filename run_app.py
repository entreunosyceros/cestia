#!/usr/bin/env python3
"""Punto de entrada de CestIA — crea/usa .venv, instala deps y lanza la app."""
...
def obtener_directorio_venv() -> Path:
    ...
    - Instalación de solo lectura: ~/.local/share/cestia/.venv
    - CESTIA_VENV: ruta explícita
    """
    override = os.environ.get("CESTIA_VENV")
    if override:
...
def iniciar_app() -> int:
    from cestia import principal as ejecutar_cestia

    try:
        return ejecutar_cestia()
    except KeyboardInterrupt:
        print(
            "\nCestIA: aplicación cerrada con Ctrl+C.\n",
            flush=True,
        )
        return 0
...
def principal() -> int:
    ...
    if ejecutando_en_venv():
        try:
            ...
            print(f"Error al iniciar CestIA: {exc}", file=sys.stderr)
            traceback.print_exc()
            return 1
    ...
    try:
        ...
        print(
            ...
            "- Recrea el entorno: borra ~/.local/share/cestia/.venv "
        ...
        print(
            "\nCestIA: interrupción por teclado (Ctrl+C) durante la preparación.\n"
